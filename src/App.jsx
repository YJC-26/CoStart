    ...
    <div style={{ background: C.surface, borderRadius: "20px", border: `1px solid ${C.border}`, padding: isMobile ? "28px 24px" : "36px", boxShadow: C.shadow, width: "100%", maxWidth: isMobile ? "100%" : "380px" }}>
      ...
      {isMobile && (
        <div style={{ textAlign: "center", marginBottom: "28px" }}>
          ...
          <h1 style={{ fontSize: "24px", fontWeight: "800", color: C.text, letterSpacing: "-0.03em", lineHeight: "1.2", marginBottom: "8px" }}>
            AI 팀과 함께<br /><span style={{ color: C.accent }}>창업을 시작하세요</span>
          </h1>
          <p style={{ fontSize: "13px", color: C.textSub, lineHeight: "1.6" }}>사업자·창업 준비자 맞춤 AI 팀</p>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "20px" }}>
        ...
        {/* Google */}
        <button onMouseEnter={() => setHovered("google")} onMouseLeave={() => setHovered(null)} onClick={() => onLogin("google")} style={{
          width: "100%", padding: "14px 18px", borderRadius: "12px", cursor: "pointer",
          border: `1.5px solid ${hovered === "google" ? "#4285F4" : C.border}`,
          background: hovered === "google" ? "rgba(66,133,244,0.06)" : C.surface2,
          display: "flex", alignItems: "center", gap: "12px", transition: "all 0.2s",
          transform: hovered === "google" ? "translateY(-1px)" : "none",
          boxShadow: hovered === "google" ? "0 4px 12px rgba(66,133,244,0.15)" : "none",
        }}>
        ...
        {/* Kakao */}
        <button onMouseEnter={() => setHovered("kakao")} onMouseLeave={() => setHovered(null)} onClick={() => onLogin("kakao")} style={{
          width: "100%", padding: "14px 18px", borderRadius: "12px", cursor: "pointer",
          ...
          boxShadow: hovered === "kakao" ? "0 4px 12px rgba(254,229,0,0.4)" : "none",
        }}>
          <svg width="20" height="20" viewBox="0 0 24 24">
        ...
        {/* Email */}
        <button onMouseEnter={() => setHovered("email")} onMouseLeave={() => setHovered(null)} onClick={() => onLogin("email")} style={{
          width: "100%", padding: "14px 18px", borderRadius: "12px", cursor: "pointer",
          border: `1.5px solid ${hovered === "email" ? C.accentBorder : C.border}`,
          background: hovered === "email" ? C.accentSoft : "transparent",
      ...
      <p style={{ fontSize: "11px", color: C.textMuted, textAlign: "center", lineHeight: "1.7" }}>
        계속 진행하면 <span style={{ color: C.accent, cursor: "pointer" }}>이용약관</span>과{" "}
        <span style={{ color: C.accent, cursor: "pointer" }}>개인정보처리방침</span>에 동의합니다
      </p>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "80px 60px", position: "relative", zIndex: 1 }}>
        <div style={{ maxWidth: "460px" }}>
          ...
          <h1 style={{ fontSize: "clamp(36px, 4vw, 52px)", fontWeight: "800", color: C.text, lineHeight: "1.15", letterSpacing: "-0.03em", marginBottom: "18px" }}>
            AI 팀과 함께<br /><span style={{ color: C.accent }}>창업을 시작하세요</span>
          </h1>
          <p style={{ fontSize: "16px", color: C.textSub, lineHeight: "1.75", marginBottom: "48px" }}>
            사업자와 창업 준비자를 위한<br />맞춤형 AI 팀 플랫폼이에요
          </p>

          <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
            {[
              { emoji: "🏪", title: "기존 사업자", desc: "재무·세무·마케팅·CS를 AI가 함께 처리해요" },
              { emoji: "🚀", title: "창업 준비자", desc: "아이디어 검증부터 사업 시작까지 길라잡이" },
              { emoji: "🤖", title: "맞춤 AI 팀", desc: "업종·상황에 맞는 전문 에이전트가 구성돼요" },
            ].map((f, i) => (
...
export default function App() {
  ...
  const handleLogin = () => setStage("type");
